...
#define WAKE_COMMAND_SLAVE_PIN 9       
...
void wakeCommandSlave() {
  logEvent("Waking COMMAND slave...");
  digitalWrite(WAKE_COMMAND_SLAVE_PIN, LOW);  // Set to LOW and keep it low for LOW level interrupt
  logEvent("Wake signal sent (LOW level), WAKE_COMMAND_SLAVE_PIN state: " + String(digitalRead(WAKE_COMMAND_SLAVE_PIN)));
  total_wakeup++;
}
...
void endWakeCommandSlave() {
  digitalWrite(WAKE_COMMAND_SLAVE_PIN, HIGH);  // Reset to HIGH after command is sent
  logEvent("Wake command signal ended (back to HIGH)");
}
...
void setup() {
  ...
  pinMode(WAKE_COMMAND_SLAVE_PIN, OUTPUT);
  digitalWrite(WAKE_SLAVE_PIN, HIGH);  // Initial HIGH for LOW level interrupt
  digitalWrite(WAKE_COMMAND_SLAVE_PIN, HIGH);  // Initial HIGH for LOW level interrupt
